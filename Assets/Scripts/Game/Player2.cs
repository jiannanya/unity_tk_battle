...
public class Player2 : MonoBehaviour {
	...
	private void Attack () { //坦克的攻击方法
		if (Input.GetKeyDown (KeyCode.Space)) {
			...
			go.GetComponent<Bullect> ().isPlayer1Bullect = false;
			timeVal = 0;
		}
	}
	...
	private void Move () {
		...
		if (Mathf.Abs (v) > 0.07 || Mathf.Abs (h) > 0.07) {
			if (!isHasMove) { //移动且第一次移动才切换动画，切换之后unity会自动稳定到过度后的动画
				this.anim.SetTrigger ("Player2AdleToMove");
			}
			isHasMove = true;
		} else {
			if (isHasMove) {
				this.anim.SetTrigger ("Player2MoveToAdle");
			}
			isHasMove = false;
		}
		//音效
	...
	private void Die () {
		...
		PlayerManager.Instance.isDied2 = true;
		//产生爆炸特效
